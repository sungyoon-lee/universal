...
if __name__ == '__main__':
    ...
    with tf.device(device):
        ...
        image_denoised_5 = denoise_wavelet(undo_image_avg(image_perturbed[0,:,:,:]).astype(dtype='uint8')/255, multichannel=True)
        label_denoised_5 = np.argmax(f(image_denoised_5), axis=1).flatten()
        str_label_denoised_5 = labels[np.int(label_denoised_5)-1].split(',')[0]
        
        image_denoised = denoise_tv_chambolle(image_perturbed, weight=10, multichannel=True)
        label_denoised = np.argmax(f(image_denoised), axis=1).flatten()
        str_label_denoised = labels[np.int(label_denoised)-1].split(',')[0]

        image_denoised_20 = denoise_tv_chambolle(image_perturbed, weight=20, multichannel=True)
        label_denoised_20 = np.argmax(f(image_denoised_20), axis=1).flatten()
        str_label_denoised_20 = labels[np.int(label_denoised_20)-1].split(',')[0]

        print('original   : ', estimate_sigma(image_original, multichannel=True, average_sigmas=True))       
        print('ROF_5      : ', estimate_sigma(image_ROF_5, multichannel=True, average_sigmas=True))
        print('ROF        : ', estimate_sigma(image_ROF, multichannel=True, average_sigmas=True))
        print('ROF_20     : ', estimate_sigma(image_ROF_20, multichannel=True, average_sigmas=True))
        print('perturbed  : ', estimate_sigma(image_perturbed, multichannel=True, average_sigmas=True))
        print('denoised_5 : ', estimate_sigma(image_denoised_5, multichannel=True, average_sigmas=True))
        print('denoised   : ', estimate_sigma(image_denoised, multichannel=True, average_sigmas=True))
        print('denoised_10: ', estimate_sigma(image_denoised_20, multichannel=True, average_sigmas=True))
        ...
        plt.subplot(2, 4, 2)
        plt.imshow(image_ROF_5[ :, :, :], interpolation=None)
        plt.title(str_label_ROF_5)

        plt.subplot(2, 4, 3)
        plt.imshow(undo_image_avg(image_ROF[0, :, :, :]).astype(dtype='uint8'), interpolation=None)
        plt.title(str_label_ROF)

        plt.subplot(2, 4, 4)
        plt.imshow(undo_image_avg(image_ROF_20[0, :, :, :]).astype(dtype='uint8'), interpolation=None)
        plt.title(str_label_ROF_20)
        ...
        plt.subplot(2, 4, 6)
        plt.imshow(image_denoised_5[:, :, :], interpolation=None)
        plt.title(str_label_denoised_5)
        ...
        plt.subplot(2, 4, 8)
        plt.imshow(undo_image_avg(image_denoised_20[0, :, :, :]).astype(dtype='uint8'), interpolation=None)
        plt.title(str_label_denoised_20)
